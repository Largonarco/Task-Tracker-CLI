...
This is my solution for the problem and no external packages were used.
This is also my first ever cli program so might not use the best practices but all the tests pass and the functionality is as specified.

I have used the fs module for reading and writing to files syncronously.
loadData() and loadCompleted() are just functions to read from the "task.txt" and "completed.txt" files.
I made them as different functions instead passing arguments to fs.readFileSync() is because one is sorted by priority and the other isn't and to make it look tidier rather than cramping all the code.
I hope the code isn't too confusing to understand and additinal info is attached to the functions
